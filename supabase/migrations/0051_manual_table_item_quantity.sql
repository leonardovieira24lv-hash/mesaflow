-- FORKO — quantidade em item avulso.
-- `p_amount` continua sendo valor UNITÁRIO; o total da ordem é calculado
-- no banco para preservar caixa, histórico, impressão e total da mesa.

drop function if exists public.add_manual_table_item(uuid, uuid, text, numeric, text);
...
begin
  ...
  insert into public.orders (
    restaurant_id, table_id, order_session_id, status, total_amount, notes
  )
  values (
    p_restaurant_id, p_table_id, v_session_id, 'delivered', v_total, null
  )
  ...
  values (
    v_order_id, null, trim(p_name), v_unit_amount, p_quantity,
  ...
  return query select v_order_id, v_session_id, v_total;
end;
$$;

revoke all on function public.add_manual_table_item(uuid, uuid, text, numeric, integer, text)
  from public, anon, authenticated;
grant execute on function public.add_manual_table_item(uuid, uuid, text, numeric, integer, text)
  to service_role;
